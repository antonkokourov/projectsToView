...
def doInputText():
    ...
    while True:
        inputText = input("Введите слово для поиска его анаграмм: ").lower()
        if inputText == '#':
            sys.exit()
        iText = sorted([i for i in inputText if i.isalpha()])

        if inputText == "":
            print("!Вы ничего не ввели")
        else:
            return iText, inputText
...
def main():
    ...
    while True:
        ...
        lettersInText = deletingLetters(lettersInText, choiceWord)

        if sizeResult >= sizeText:
            break
        print("Сейчас фраза: ", *result)
        print("Остались буквы: ", *list(lettersInText.elements()), sep=', ')
